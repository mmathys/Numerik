...
struct sparse_vec
{
	...
	static sparse_vec cwise_mult(const sparse_vec &a, const sparse_vec &b)
	{
		...
		while (hasMore)
		{
			// set next index. if not found, fuck it.
			if (a_p > a.duplets.size() - 1 || b_p > b.duplets.size() - 1)
			{
				hasMore = false;
				continue;
			}

			int a_ind = a.duplets[a_p].ind;
			int b_ind = b.duplets[b_p].ind;
			...
			{
				out.append(a_ind, a.get_val(a_ind) * b.get_val(b_ind));
				a_p++;
				b_p++;
			}
		}
